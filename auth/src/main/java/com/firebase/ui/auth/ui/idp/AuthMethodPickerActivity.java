...
public class AuthMethodPickerActivity extends AppCompatBase {
    ...
    private void populateIdpList(List<IdpConfig> providerConfigs,
        ...
        for (int i = 0; i < providerConfigs.size(); i++) {
            IdpConfig idpConfig = providerConfigs.get(i);
            ...
            if (idpConfig.getProviderId().equals(FacebookAuthProvider.PROVIDER_ID)) {
                View facebookNote = getLayoutInflater().inflate(
                        R.layout.fui_idp_button_facebook_note, mProviderHolder, false);
                mProviderHolder.addView(facebookNote);
            }

            if (providerConfigs.size() > 1 && i != providerConfigs.size() - 1) {
                View divider = getLayoutInflater().inflate(R.layout.fui_idp_auth_button_divider,
                        mProviderHolder, false);
                mProviderHolder.addView(divider);
            }
        }
    }
